...
def fire(source_name):
    ...
    def hnx(el):
        ...
        try:
            ...
            eprint('Skipping ' + el + ': ValueError thrown')
            return
        ...
        with pd.option_context('display.max_rows', None, 'display.max_columns', None, 'dropna', True), pd.ExcelWriter('stock_' + datetime.today().strftime('%d-%m-%Y') + '.xlsx', engine='xlsxwriter') as ewriter:
            last_row_len = 0
